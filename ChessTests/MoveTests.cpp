...
TEST(MoveTests, PawnMoveTest)
{
	...
	auto whitePawn4 = game.GetBoard()->GetElement({ 6, 5 });
	...
	EXPECT_THROW(game.MovePiece(blackPawn1->GetPosition(), { 2, 0 }), IllegalMoveException);
	...
	game.MovePiece(whitePawn1->GetPosition(), { 4, 1 });
	game.MovePiece(whitePawn2->GetPosition(), { 4, 4 });

	//black pawn captures the white pawn
	EXPECT_NO_THROW(game.MovePiece(blackPawn1->GetPosition(), { 4, 1 }));
	EXPECT_EQ(game.GetBoard()->GetElement({ 4,1 })->GetColor(), Color::BLACK);

	// illegal move
	EXPECT_NO_THROW(game.MovePiece(whitePawn3->GetPosition(), { 4,3 }));
	EXPECT_NO_THROW(game.MovePiece(whitePawn4->GetPosition(), { 4,5 }));
	EXPECT_THROW(game.MovePiece(blackPawn2->GetPosition(), { 4,4 }), IllegalMoveException);
	EXPECT_NO_THROW(game.MovePiece(blackPawn2->GetPosition(), { 4,5 }));

	// out of bounds
	EXPECT_THROW(game.MovePiece(whitePawn2->GetPosition(), { 1, 9 }), OutOfBoundsException);
